# Python script for running the Single run version of the Custom Controller
import sys, time, os
...
def run_automation_once():
    print(f"Starting Otterize Denial Automation (single run) for namespace: {NAMESPACE_TO_WATCH}")
    sys.stdout.flush()


    try:
        print(f"DEBUG: Calling list_namespaced_custom_object for namespace: {NAMESPACE_TO_WATCH}")
        sys.stdout.flush()
        client_intents = custom_objects_api.list_namespaced_custom_object(
            group="k8s.otterize.com",
            version="v2beta1",
            namespace=NAMESPACE_TO_WATCH,
            plural="clientintents"
        )
        print(f"DEBUG: Raw response from list_namespaced_custom_object: {client_intents}")
        sys.stdout.flush()

        if 'items' not in client_intents or not client_intents['items']:
            print("DEBUG: No ClientIntents found in the response 'items' list. Skipping processing loop.")
            sys.stdout.flush()
        else:
            print(f"DEBUG: Found {len(client_intents['items'])} ClientIntents. Processing them now...")
            sys.stdout.flush()
            for obj in client_intents['items']:
                intent_name = obj['metadata']['name']
                intent_namespace = obj['metadata']['namespace']

                print(f"Processing ClientIntent '{intent_name}' in namespace '{intent_namespace}'")
                sys.stdout.flush()
        ...
        print(f"Error fetching ClientIntents: {e}")
        sys.stdout.flush()

    print("Immediate processing complete. Exiting script.")
    sys.stdout.flush()
